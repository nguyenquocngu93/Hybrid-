...
function searchBitsearch(query, maxResults) {
 return fetch(BITSEARCH_API + '?q=' + encodeURIComponent(query) + '&limit=' + (maxResults || 20), { timeout: 60000 })
...
function searchTorrentCsv(query, maxResults) {
 return fetch(TORRENTCSV_API + '?q=' + encodeURIComponent(query) + '&size=' + (maxResults || 20), { timeout: 60000 })
...
function handleStream(type, id, cfg, res, pub) {
 ...
 // BITSEARCH + TORRENT CSV
 if (cfg.bitsearchEnabled) {
 var metaType = (type === 'series') ? 'tv' : 'movie';
 fetch('https://api.themoviedb.org/3/find/' + imdbId + '?api_key=' + TMDB_API_KEY + '&external_source=imdb_id', { timeout: 60000 })
 .then(function(r) { return r.ok ? r.json() : {}; })
 .then(function(data) {
 var results = data[metaType + '_results'] || [];
