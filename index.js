...
/**
 * Xác định loại torrent: 'pack' hoặc 'episode'
 * Pack = chứa nhiều tập (range như S01E01-E18, S01 Complete, không có SxxExx đơn lẻ)
 * Episode = chỉ 1 tập đơn lẻ (S01E02)
 */
function classifyKnabenTorrent(title) {
  // Range pattern: S01E01-E18, S01E01-18, S2E1-18, S02E01-E22 ...
  var rangePattern = /\bS(\d{1,2})E(\d{1,3})\s*[-–]\s*E?(\d{1,3})\b/i;
  // Season pack (không có episode): S01, Season 1, S01 Complete
  ...
  // Single episode: S01E02 (không theo sau bởi range)
  ...
  // Nếu có SxxExx nhưng theo sau là dash range thì pack (đã xử lý trên)
  // Nếu chỉ có season mà không có episode → pack
  if (seasonPackPattern.test(title)) return 'pack';
  // Không xác định được → mặc định pack (thường là full pack)
  return 'pack';
}

/**
 * Lấy season number từ title torrent Knaben
 * Trả về array các season number xuất hiện trong title
 */
function extractSeasonsFromTitle(title) {
  ...
  if (rangeMatch) {
    rangeMatch.forEach(function(m) {
      var n = parseInt(m.replace(/[^0-9]/g,''));
      // Lấy số đầu tiên sau S
  ...
  if (seasonWordMatch) {
    seasonWordMatch.forEach(function(m) {
      var sm = m.match(/(\d{1,2})/); if (sm) { var sn = parseInt(sm[1]); if (seasons.indexOf(sn) === -1) seasons.push(sn); }
    });
  }
  // S01 standalone (không theo sau E)
...
function searchKnaben(query, maxResults, type, preferPack, season, episode) {
  ...
  // Nếu tìm tập lẻ, thêm SxxExx vào query
...
function handleStream(type, id, cfg, res, pub) {
  ...
  if (cfg.knabenEnabled) {
    Promise.all([
      ...
      // Luôn dùng tiếng Anh cho Knaben
    ...
    .then(function(results) {
      ...
      results.forEach(function(t) {
        ...
        // ── Phân loại: pack hay tập lẻ ──
        var torrentType = classifyKnabenTorrent(title); // 'pack' | 'episode'
        var isPack = (torrentType === 'pack');
        var isSingleEpisode = (torrentType === 'episode');

        if (type === 'series') {
          // ── Lọc season không khớp ──
          if (season > 0) {
            var titleSeasons = extractSeasonsFromTitle(title);

            // Nếu title có thông tin season và không chứa season đang xem → bỏ qua
            if (titleSeasons.length > 0 && titleSeasons.indexOf(season) === -1) {
              console.log('[Knaben] ⏭️ Bỏ qua "' + title + '" - season ' + titleSeasons.join(',') + ' ≠ ' + season);
              return;
            }

            // Nếu là tập lẻ (episode), kiểm tra đúng season + episode
            if (isSingleEpisode) {
              var epMatch = title.match(/\bS(\d{1,2})E(\d{1,3})\b/i);
              if (epMatch) {
                var epSeason = parseInt(epMatch[1]);
                var epEpisode = parseInt(epMatch[2]);
                if (epSeason !== season) {
                  console.log('[Knaben] ⏭️ Bỏ qua tập lẻ sai season: S' + epSeason + 'E' + epEpisode + ' (cần S' + season + ')');
                  return;
                }
                // Nếu đang lọc tập lẻ chính xác thì check episode number
                if (!cfg.preferPack && epEpisode !== episode) {
                  console.log('[Knaben] ⏭️ Bỏ qua tập lẻ sai episode: E' + epEpisode + ' (cần E' + episode + ')');
                  return;
                }
              }
            }

            // Nếu prefer pack và torrent là tập lẻ → bỏ qua
            if (cfg.preferPack && isSingleEpisode) return;

            // Nếu không prefer pack và torrent là pack → bỏ qua
            if (!cfg.preferPack && isPack) return;
          }
        }

        var sizeGB = t.sizeGB.toFixed(2);
        var streamName = '🟠 Knaben';

        // Badge nhãn đúng
        var badge = '';
        if (type === 'series') {
          badge = isPack ? '📦 PACK | ' : '🎬 TẬP LẺ | ';
        }
        ...
        var url;
        if (type === 'movie') {
          url = ts + '/stream/' + encodeURIComponent(title) + '?link=' + encodeURIComponent(t.magnet) + '&index=0&play';
        } else {
          if (isPack) {
            // Pack → dùng /play để tìm đúng tập trong pack
            url = pub + '/play?magnet=' + encodeURIComponent(t.magnet) + '&s=' + season + '&e=' + episode + '&title=' + encodeURIComponent(title) + '&ts=' + encodeURIComponent(ts);
          } else {
            // Tập lẻ → stream thẳng index 0
            url = ts + '/stream/' + encodeURIComponent(title) + '?link=' + encodeURIComponent(t.magnet) + '&index=0&play';
          }
        }
  ...
  if (cfg.jacredEnabled) {
    ...
    searchJacred(imdbId, type, cfg.maxResults || 30, commonSort, apiUrl).then(function(results) {
      results.forEach(function(t) {
        ...
        if (type === 'movie') {
          streams.push({ name: streamName, title: streamTitle, url: ts + '/stream/' + encodeURIComponent(t.title) + '?link=' + encodeURIComponent(t.magnet) + '&index=0&play', behaviorHints: { notWebReady: true, bingeGroup: 'jacred-' + idClean } });
        } else {
          streams.push({ name: streamName, title: streamTitle, url: pub + '/play?magnet=' + encodeURIComponent(t.magnet) + '&s=' + season + '&e=' + episode + '&title=' + encodeURIComponent(t.title) + '&ts=' + encodeURIComponent(ts), behaviorHints: { notWebReady: true, bingeGroup: 'jacred-' + idClean } });
        }
      });
      sendResponse();
...
server.listen(PORT, '0.0.0.0', function() {
  console.log('\n✅ Hybrid Addon v6.6.2: http://localhost:' + PORT);
